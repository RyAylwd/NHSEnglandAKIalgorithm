 ...
 *NB change the forvalues loop i=1/n into the maximum n number of rows per person 
...
*OPTIONAL: split dataset to reduce computational load if there are large number of patients or alerts. If this is required, create a master do file that executes the algorithm in each split dataset separately, and then append all the split datasets once complete. 
cd "Split 1" // change working directory as desired 
keep if pat<= // for example if you have 100 000 patients, you might want to split into 50 000 each. it is faster to split and combine than to execute all patients at the same time
save "split1.dta", replace
  
*Reshape wide, so you have date1, creatinine1, date2, creatinine2 etc
use "split1.dta", clear // If splitting is not required, change the using data to "data.dta"
preserve 
...
*duplicate dataset for each part below (will be re-combined at the end)
...
*set lab reference interval - this depends on the assay used by the lab and patient sex
gen lab_upper_range = 
gen lab_lower_range = 
...
drop creat creatinine1-diff7_yn*
...
drop creat creatinine1-diff8365_yn*
...
*retain the creatinine rv1/2 as appropriate if ratio1/2 >1.5 and drop the value associated with lower ratio
...
gen creat_diff = creatinine-rv // called 'D' in algorithm

*generate central alerts
gen alert=0 

***********
*6* 48h criteria AKI 1

*change date in milliseconds to hours
...
*drop redundant variables
drop creatinine1-date* 
drop date_hour0-diff48h*
